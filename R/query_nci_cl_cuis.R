#' Query NCI metathesaurus for all "CL" cuis
#' @import mySeagull
#' @export

query_nci_cl_cuis <-
        function(nci_code_cui_map_table = "nci_code_cui_map_201808") {
                conn_to_nci_metathesaurus <- mySeagull::connect_to_local_postgres(dbname = "nci_metathesaurus")

                sql_statement <- paste0("SELECT * FROM ", nci_code_cui_map_table, " WHERE nci_cui1 LIKE 'CL%' OR nci_cui2 LIKE 'CL%';")
